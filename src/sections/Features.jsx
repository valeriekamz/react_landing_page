            ...
            <div className="container">
                <div
                    ...
                    {features.map(({id, icon, caption, title, text, button}) => (
                        <div
                            key={id}
                            className="relative z-2 md:px-10 px-5 md:pb-10 pb-5 flex-50 max-md:g7 max-md:border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex320"
                        >
                        ...
                        {details.map(({ id, icon, title }) => (
                            <li key={id} className="relative pt-16 px-4 pb-14">
                                ...
                                <div className="flex items-center justify-center mx-auto mb-3 border-2 border-s2 rounded-full hover:border-s4 transition-all duration-500 shadow-500 size-20">
                                    <img src={icon} alt={title} className="size-17/20 objeect-contain z-20" />
                                </div>
